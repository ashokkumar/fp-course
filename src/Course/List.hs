...
flatten ::
  List (List a)
  -> List a
flatten lists = foldRight (++) Nil lists
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight seqe (Full Nil)

seqe :: Optional a -> Optional (List a) -> Optional (List a)
seqe Empty _ = Empty
seqe _ Empty = Empty
seqe (Full x) (Full l) = Full (x :. l)
...
find ::
  ...
  -> Optional a
find _ Nil = Empty
find fn (h:.t) = bool (find fn t) (Full h) (fn h)
